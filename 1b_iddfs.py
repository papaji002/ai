    ...
    if problem.goal_test(node.state):
        ...
        for child in node.expand(problem):
            ...
            if result == 'cut-off':
                ...
                return result
        return 'cut-off' if cutoff_occured else 'Not-Found'
...
def iterative_deepening_search(problem, limit):
    for depth in range(0, limit):
        print(f"Checking with depth:{depth}")
        result = depth_limited_search(problem, depth)
        print(f"result:{result}")

romania_map = Graphs(dict({
...
# print("\n--searching from arad to Neamt with level 8.....")
# romania_problem = GraphProblem('Arad','Neamt', romania_map)
# iterative_deepening_search(romania_problem,8)
